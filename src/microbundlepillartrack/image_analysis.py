...
def hello_microbundle_pillar_track() -> str:
...
def sub_division_mask(mask: np.ndarray, box: np.ndarray) -> float:
    """Given a mask and a box. Will return the number of mask pixels within the box."""
    r0, r1, c0, c1 = box_to_bound(box)
    mask_in_div = np.sum(mask[r0:r1,c0:c1])
    return mask_in_div


def compute_local_coverage(mask: np.ndarray, track_0_pts: np.ndarray, clip_fraction: float = 0.3, sub_division_dim_pix: int = 20) -> List:
    ...
    new_mask = remove_pillar_region(rot_mask, clip_fraction = clip_fraction, clip_columns = True, clip_rows = True)
...
def adjust_qualityLevel(feature_params: dict, img_uint8: np.ndarray, mask: np.ndarray, min_coverage: Union[float, int]):
    """Given feature parameters, an image, and a mask. Will automatically update the feature quality.
...
def adjust_feature_param_dicts(feature_params: dict, img_uint8: np.ndarray, mask: np.ndarray, clip_fraction: float = 0.3, min_coverage: Union[float, int] = 40, min_local_coverage: Union[float, int] = 50) -> dict:
    ...
    (min_local_coverage refers to the number of pixels that should be attributed to 1 tracking point within each mask subdivision)"""
    _,_ = adjust_qualityLevel(feature_params, img_uint8, mask, min_coverage)
    track_points_0 = mask_to_track_points(img_uint8, mask, feature_params)
    local_coverage = compute_local_coverage(mask,track_points_0,clip_fraction)
...
def track_all_steps_with_adjust_param_dicts(img_list_uint8: List, mask: np.ndarray, clip_fraction: float = 0.3) -> dict:
    ...
    img_0 = img_list_uint8[0]
    feature_params = adjust_feature_param_dicts(feature_params, img_0, mask, clip_fraction)
    tracker_0, tracker_1 = track_all_steps(img_list_uint8, mask, feature_params, lk_params)
    disp_abs_mean, disp_abs_all, _, _  = compute_abs_position_timeseries(tracker_0, tracker_1)
    max_disp_abs = np.max(disp_abs_all)
    max_disp_abs_mean = np.max(disp_abs_mean)
...
def compute_valleys(timeseries: np.ndarray) -> np.ndarray:
    """Given a timeseries. Will compute valleys."""
