...
def test_hello_world():
    ...
    res = ia.hello_microbundle_pillar_track()
...
def test_compute_local_coverage():
    ...
    clip_frac = 0.5
    sub_division_dim_pix = 20
    all_local_coverage = ia.compute_local_coverage(mask, track_points_0, clip_frac, sub_division_dim_pix)
...
def test_track_all_steps_with_adjust_param_dicts_warping():
    ...
    diff_0 = np.abs(tracker_0[:, 0] - tracker_0[:, -1])
    diff_1 = np.abs(tracker_1[:, 0] - tracker_1[:, -1])
    np.savetxt(str(folder_path)+'/diff0.txt',diff_0)
    np.savetxt(str(folder_path)+'/diff1.txt',diff_1)
...
def test_subpixel_abs_position_timeseries():
    ...
    with warnings.catch_warnings(record=True) as record:
        ...
        tracker_0, tracker_1 = ia.track_all_steps_with_adjust_param_dicts(img_list_uint8, mask)
    disp_abs_mean, _, _, _ = ia.compute_abs_position_timeseries(tracker_0, tracker_1)
    assert np.max(disp_abs_mean) < 1
    assert len(record) == 1
...
def test_compute_beat_frequency():
    ...
    assert np.isclose(freq,fps*4/250, atol=.01)
    li = 10 * [-0.99] + list(timeseries) + 10 * [-0.99]
    timeseries = np.asarray(li)
    info = ia.compute_valleys(timeseries)
    fps = 2
    ...
    timeseries = np.sin(x / (np.pi * 40.0) - np.pi / 2.0)
    fps = 1
    info = ia.compute_valleys(timeseries)
    freq = ia.compute_beat_frequency(info, fps)
    assert np.isclose(freq, fps *1 / (np.pi * 40.0), atol=.01)
...
def test_axis_from_mask():
    ...
    # rotated example
    mask = np.zeros((100, 100))
    for kk in range(10, 60):
        mask[kk, kk + 20:kk + 30] = 1
    center_row, center_col, vec = ia.axis_from_mask(mask)
    assert np.isclose(center_row, (10 + 60) / 2.0, atol=4)
...
def test_rotate_test_img():
    ...
    img_path, rot_img, _, _ = ia.rotate_test_img(folder_path, tiff_list, ang, center_row, center_col, rot_mat)
    assert img_path.is_file()
    assert rot_img.shape == tiff_list[0].shape
...
def test_rotate_non_square_test_img():
    ...
    vec = [1,1]
    rot_mat, ang = ia.rot_vec_to_rot_mat_and_angle(vec)
    img_r, img_c = tiff_list[0].shape
    max_size = np.max([img_r,img_c])
    center_row = img_r/2
    center_col = img_c/2
    img_path, rot_img, translate_r, translate_c = ia.rotate_test_img(folder_path, tiff_list, ang, center_row, center_col, rot_mat)
    assert img_path.is_file()
    assert rot_img.shape == (max_size,max_size)
    assert translate_r == (max_size - img_r)//2
    assert translate_c == (max_size - img_c)//2
